...
import com.Devlex.iWifiFileTransfer.BaseActivity.State;
...
import com.Devlex.iWifiFileTransfer.libcore.io.MultiMap;
...
public class RootsCache {
    ...
    private class RootsChangedObserver extends ContentObserver {
        ...
        public void onChange(boolean selfChange, Uri uri) {
            ...
            if (LOGD) Log.d(TAG, "TAG  Updating roots due to change at " + uri);
    ...
    private void loadStoppedAuthorities() {
        ...
        synchronized (mLock) {
            for (String authority : mStoppedAuthorities) {
                if (LOGD) Log.d(TAG, "TAG  Loading stopped authority " + authority);
    ...
    private class UpdateTask extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground() {
            ...
            Log.d(TAG, "TAG  Update found " + mTaskRoots.size() + " roots in " + delta + "ms");
            synchronized (mLock) {
                mRoots = mTaskRoots;
        ...
        private void handleDocumentsProvider(ProviderInfo info) {
            ...
            if ((info.applicationInfo.flags & ApplicationInfo.FLAG_STOPPED) != 0) {
                if (LOGD) Log.d(TAG, "TAG  Ignoring stopped authority " + info.authority);
            ...
            if (mAuthority != null && !mAuthority.equals(info.authority)) {
                synchronized (mLock) {
                    if (mTaskRoots.putAll(info.authority, mRoots.get(info.authority))) {
                        if (LOGD) Log.d(TAG, "TAG  Used cached roots for " + info.authority);
                        cacheHit = true;
                    }
                }
            }
    ...
    private Collection<RootInfo> loadRootsForAuthority(ContentResolver resolver, String authority) {
        if (LOGD) Log.d(TAG, "TAG  Loading roots for " + authority);
