...
import com.Devlex.iWifiFileTransfer.DialogFragment;
...
import static com.Devlex.iWifiFileTransfer.provider.ExternalStorageProvider.ROOT_ID_SECONDARY;
...
public class SAFManager {
    ...
    public static String getRootUri(Uri uri) {
        if (isTreeUri(uri)) {
            return com.Devlex.iWifiFileTransfer.model.DocumentsContract.getTreeDocumentId(uri);
        }
        return com.Devlex.iWifiFileTransfer.model.DocumentsContract.getDocumentId(uri);
    }

    private static Uri buildDocumentUriMaybeUsingTree(Uri uri, String docId) {
        return com.Devlex.iWifiFileTransfer.model.DocumentsContract.buildDocumentUriMaybeUsingTree(uri, docId);
    }

    private static boolean isTreeUri(Uri uri) {
        return com.Devlex.iWifiFileTransfer.model.DocumentsContract.isTreeUri(uri);
    }
    ...
    public static void takeCardUriPermission(final Activity activity, RootInfo root, DocumentInfo doc) {
        ...
        if(Utils.hasNougat()){
            ...
            StorageVolume storageVolume = storageManager.getStorageVolume(new File(DocumentInfo.path));
