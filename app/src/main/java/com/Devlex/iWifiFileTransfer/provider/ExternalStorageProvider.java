...
package com.Devlex.iWifiFileTransfer.provider;
...
import com.Devlex.iWifiFileTransfer.DocumentsApplication;
import com.Devlex.iWifiFileTransfer.R;
import com.Devlex.iWifiFileTransfer.cursor.MatrixCursor;
import com.Devlex.iWifiFileTransfer.cursor.MatrixCursor.RowBuilder;
import com.Devlex.iWifiFileTransfer.libcore.io.IoUtils;
import com.Devlex.iWifiFileTransfer.CrashReportingManager;
import com.Devlex.iWifiFileTransfer.misc.DiskInfo;
import com.Devlex.iWifiFileTransfer.misc.FileUtils;
import com.Devlex.iWifiFileTransfer.misc.MimePredicate;
import com.Devlex.iWifiFileTransfer.misc.ParcelFileDescriptorUtil;
import com.Devlex.iWifiFileTransfer.misc.StorageUtils;
import com.Devlex.iWifiFileTransfer.misc.StorageVolume;
import com.Devlex.iWifiFileTransfer.misc.Utils;
import com.Devlex.iWifiFileTransfer.misc.VolumeInfo;
...
import static com.Devlex.iWifiFileTransfer.DocumentsApplication.isTelevision;
import static com.Devlex.iWifiFileTransfer.misc.FileUtils.getTypeForFile;
import static com.Devlex.iWifiFileTransfer.model.DocumentInfo.getCursorString;
import static com.Devlex.iWifiFileTransfer.provider.UsbStorageProvider.ROOT_ID_USB;
...
public class ExternalStorageProvider extends StorageProvider {
    ...
    public void updateRoots() {
        synchronized (mRootsLock) {
            ...
            includeOtherRoot();
            includeBookmarkRoot();
            Log.d(TAG, "TAG  After updating volumes, found " + mRoots.size() + " active roots");
            notifyRootsChanged(getContext());
        }
    }
    public static String sdcard="";
    public static String intcard="";
    ...
    private void updateVolumesLocked() {
        ...
        for (StorageVolume storageVolume : storageUtils.getStorageMounts()) {
            ...
            if (storageVolume.isPrimary()) {
                ...
                intcard=storageVolume.getPath();
                Log.d(TAG, "TOG  File path rootId " + rootId );
                ...
                Log.d(TAG, "TOG  File path ROOT_ID_SECONDARY rootId " + rootId );
                Log.d(TAG, "TOG  File path                 storageVolume.getPath()\n rootId " +       storageVolume.getPath()  );
                sdcard=storageVolume.getPath();
                ...
                Log.d(TAG, "TOG  File path  " + storageVolume.getPath() + "; skipping");
                continue;
            }
            ...
            try {
                ...
                Log.d(TAG, "TAG  Missing UUID for " + root);
    ...
    private void updateVolumesLocked2() {
        ...
        for (VolumeInfo volume : storageUtils.getVolumes()) {
            ...
            if (TextUtils.isEmpty(rootId)) {
                Log.d(TAG, "TAG  Missing UUID for " + volume.getId() + "; skipping");
                continue;
            }
            if (mRoots.containsKey(rootId)) {
            ...
            Log.d(TAG, "TOG  Disk for root " + rootId + " is " + disk);
            if (disk != null && disk.isSd()) {
                root.flags |= Root.FLAG_REMOVABLE_SD;
                Log.d(TAG, "TOG  Disk  root " + root + " is " + disk);
            ...
            if (volume.isPrimary()) {
                ...
                Log.d(TAG, "TOG  Disk  root " + primaryVolume );
    ...
    public String createDocument(String docId, String mimeType, String displayName)
        ...
        Log.w(TAG, "TOG 1 displayName : "+ displayName);

        Log.w(TAG, "TOG 1 displayName docid : "+ docId);

        final File parent = getFileForDocId(docId);
        Log.w(TAG, "TOG 1 displayName getFileForDocId docid : "+ parent);
        ...
        Log.w(TAG, "TOG 1 buildUniqueFile File docid : "+ file);

        DocumentFile documentFile = getDocumentFile(docId, parent);
        Log.w(TAG, "TOG 1 documentFile External : "+ documentFile);

        if (Document.MIME_TYPE_DIR.equals(mimeType)) {

            DocumentFile newFile = documentFile.createDirectory(displayName);
            Log.w(TAG, "TOG 1 DocumentFile newFile : "+ newFile);
            ...
            Log.w(TAG, "TOG 1 DocumentFile mimeType, displayName :  "+ mimeType +","+ displayName);
    ...
    public Cursor queryChildDocuments(
            String parentDocumentId, String[] projection)
            throws FileNotFoundException {
    ...
    public AssetFileDescriptor openDocumentThumbnail(
        ...
        return openOrCreateDocumentThumbnail(documentId);
    }
...
//        final ContentResolver resolver = getContext().getContentResolver();
    ...
    private void startObserving(File file, Uri notifyUri) {
        synchronized (mObservers) {
            ...
            if (LOG_INOTIFY) Log.d(TAG, "TAG  after start: " + observer);
        }
    }
    ...
    private void stopObserving(File file) {
        synchronized (mObservers) {
            ...
            if (LOG_INOTIFY) Log.d(TAG, "TAG  after stop: " + observer);
        }
    }
    ...
    private class DirectoryObserver extends FileObserver {
        ...
        public void onEvent(int event, String path) {
            if ((event & NOTIFY_EVENTS) != 0) {
                if (LOG_INOTIFY) Log.d(TAG, "TAG  onEvent() " + event + " at " + path);
