...
public class MediaDocumentsProvider extends StorageProvider {
     ...
     */
    static void onMediaStoreInsert(Context context, String volumeName, int type) {
    ...
    @Override
    public Cursor queryChildDocuments(String docId, String[] projection)
...
//        final ContentResolver resolver = getContext().getContentResolver();
        ...
        try {
            if (TYPE_IMAGES_BUCKET.equals(ident.type)) {
                ...
                return openOrCreateImageThumbnailCleared(ident.id);
            } else if (TYPE_VIDEOS_BUCKET.equals(ident.type)) {
                final long id = getVideoForBucketCleared(ident.id);
                return openOrCreateVideoThumbnailCleared(id);
            } else if (TYPE_VIDEO.equals(ident.type)) {
                return openOrCreateVideoThumbnailCleared(ident.id);
            } else if (TYPE_ALBUM.equals(ident.type)) {
                return openOrCreateAudioThumbnailCleared(ident.id);
            } else if (TYPE_AUDIO.equals(ident.type)) {
