...
public abstract class StorageProvider extends DocumentsProvider{
    ...
    protected ParcelFileDescriptor openAudioThumbnailCleared(long id)
    ...
    protected AssetFileDescriptor openOrCreateAudioThumbnailCleared(
        ...
        ParcelFileDescriptor pfd = openAudioThumbnailCleared(id);
        if (pfd == null) {
            ...
            pfd = openAudioThumbnailCleared(id);
        }
        ...
        final int orientation = queryOrientationForImage(id);
        final Bundle extras;
        if (orientation != 0) {
            extras = new Bundle(1);
    ...
    protected ParcelFileDescriptor openImageThumbnailCleared(long id)
            throws FileNotFoundException {
        final ContentResolver resolver = getContext().getContentResolver();
    ...
    protected AssetFileDescriptor openOrCreateImageThumbnailCleared(
        ...
        ParcelFileDescriptor pfd = openImageThumbnailCleared(id);
        if (pfd == null) {
            // No thumbnail yet, so generate. This is messy, since we drop the
            // Bitmap on the floor, but its the least-complicated way.
            final BitmapFactory.Options opts = new BitmapFactory.Options();
            ...
            pfd = openImageThumbnailCleared(id);
        }
    ...
    protected AssetFileDescriptor openVideoThumbnailCleared(long id)
            throws FileNotFoundException {
        final ContentResolver resolver = getContext().getContentResolver();
        Cursor cursor = null;
        try {
    ...
    protected AssetFileDescriptor openOrCreateVideoThumbnailCleared(
        ...
        AssetFileDescriptor afd = openVideoThumbnailCleared(id);
        if (afd == null) {
            // No thumbnail yet, so generate. This is messy, since we drop the
            // Bitmap on the floor, but its the least-complicated way.
            final BitmapFactory.Options opts = new BitmapFactory.Options();
            ...
            afd = openVideoThumbnailCleared(id);
        }
    ...
    protected int queryOrientationForImage(long id) {
        final ContentResolver resolver = getContext().getContentResolver();
