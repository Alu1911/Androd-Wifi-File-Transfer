...
import com.Devlex.iWifiFileTransfer.usb.UsbUtils;

import static com.Devlex.iWifiFileTransfer.DocumentsApplication.isTelevision;
import static com.Devlex.iWifiFileTransfer.misc.FileUtils.getTypeForName;
import static com.Devlex.iWifiFileTransfer.misc.MimeTypes.BASIC_MIME_TYPE;
...
public class UsbStorageProvider extends DocumentsProvider {
     ...
     */
    private static final String ACTION_USB_PERMISSION = "com.Devlex.iWifiFileTransfer.action.USB_PERMISSION";
    ...
    @Override
    public Cursor queryChildDocuments(String parentDocumentId, String[] projection) throws FileNotFoundException {
        try {
            updateSettings();
...
/*            final int accessMode = ParcelFileDescriptorUtil.parseMode(mode);
    ...
    @Override
    public String moveDocument(String sourceDocumentId, String targetParentDocumentId) throws FileNotFoundException {
        try {
    ...
    private void detachDevice(UsbDevice usbDevice) {
        for (Map.Entry<String, UsbPartition> root : mRoots.entrySet()) {
            if (root.getValue().device.equals(usbDevice)) {
                Log.d(TAG, "TAG  remove rootId " + root.getKey());
